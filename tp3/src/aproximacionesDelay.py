...
def getCalculadaPolinomioSC(mes,coef):
    return  coef[0]*(math.sin(mes)**5)+coef[1]*(math.sin(2*mes)**4)+coef[2]*(math.sin(3*mes)**3)+coef[3]*(math.cos(2*mes)**2)+coef[4]*math.cos(mes)+coef[5]
...
def prediccionPolinomioSC(datosFiles,mesesParaTrain):
    ...
    for i in range(0, len(datosFiles)):
        if(mesesParaTrain[i]==1):
            b.append(datosFiles[i])
            x.append(math.cos(i))
            x1.append(math.cos(2*i)**2)
            x2.append(math.sin(3*i)**3)
            x3.append(math.sin(2*i)**4)
            x4.append(math.sin(i)**5)
