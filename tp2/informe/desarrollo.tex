...
\begin{algorithm}
    \begin{algorithmic}[1]\parskip=2mm
        \caption{Matriz X(vector<entrada> &v,vector medias, int n)}
        \STATE{matriz X = nueva MatrizCuadrada(n)}\\
        \STATE{para i de 1 a n}\\
        \STATE{\quad para j de 1 a size(v)}
        \STATE{\quad \quad X(i,j)=v[j].vect[i] -  medias[i]) \/ \sqrt(v.size()-1)}
        \STATE{\RETURN X}
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
    \begin{algorithmic}[1]\parskip=2mm
        \caption{Matriz preY(vector<entrada> &v)}
        \STATE{matriz Y = nueva MatrizCuadrada(n)}\\
        \STATE{para i de 1 a n}\\
        \STATE{\quad para j de 1 a 10}
        \STATE{\quad \quad si v[i].label-1 = j-1}
