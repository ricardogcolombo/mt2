...
Para la implementación de esta técnica nos basamos en el paper \textbf{The Colley Matrix Explained}. La cual consiste en plantear el siguiente sistema de ecuaciones en forma matricial
...
La matriz se arriba se conoce como la Matriz de Colley , la cual es simetrica y definida positiva , ademas de ser estrictamente diagonal domimante.
Por lo tanto es posible hallar la factorizacion de Cholesky , que en el fondo no es mas que
un caso particular de la factorizacion LU , esto nos permite asegurar que a la hora de hacer eliminacion 
gaussiana no vamos a hallar 0 en la diagonal , con lo cual no hace faltar considerar pivoteo parcial.  
...
La implementación del algorimo de \textbf{Eliminación Gaussiana} que elegimos es la que se encuentra en el libro \textbf{Burden[1]}.Con el agregado de backward substitution para obtener el vector de la ecuacion Cr=b.
...
La implementación del algorimo de \textbf{Factorizacion de Cholesky} que elegimos es la que se encuentra en el libro \textbf{Burden}.
...
Siendo \textbf{n} es la cantidad de partidos jugados, \textbf{G} corresponde a partidos ganados y \textbf{T} al total de partidos jugados. \\
